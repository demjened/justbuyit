...
public abstract class EventProcessor<E extends Event<?>> {
    ...
    public EventProcessor(OAuthService oAuthService) {
     ...
     */
    public Result process(HttpServletRequest req, String eventUrl) throws Exception {
        try {
            // validate request
            oAuthService.validate(req);
        
            // sign URL and connect
            HttpURLConnection conn = oAuthService.openSignedConnection(eventUrl);
